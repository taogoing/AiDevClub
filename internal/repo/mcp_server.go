...
func (r *McpServerRepo) List(ctx context.Context, q McpServerQuery) ([]model.McpServer, int64, error) {
	...
	var ids []uint
	idQ := r.db.WithContext(ctx).Model(&model.McpServer{}).Select("id")
	...
	if q.TagID != nil {
		idQ = idQ.Where("id IN (SELECT mcp_server_id FROM mcp_server_tags WHERE tag_id = ?)", *q.TagID)
	}
	switch q.Sort {
	case "hot":
	...
	if len(ids) == 0 {
		...
		return []model.McpServer{}, total, err
	}
