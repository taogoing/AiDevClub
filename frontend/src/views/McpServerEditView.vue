<template>
  <div class="page-container">
    <h2>{{ isEdit ? '编辑 MCP Server' : '发布 MCP Server' }}</h2>
...
import { createMcpServer, updateMcpServer, getMcpServer, uploadMcpServerZip } from '@/api/mcpServer'
...
async function handleSubmit(status: string) {
  ...
  try {
    const payload: Record<string, unknown> = {
      ...
      tools_json: form.value.tools_json,
    ...
    if (zipFile.value) {
      await uploadMcpServerZip(id, zipFile.value)
    }
